...
list列表相当于java中list 集合  特点：  元素有序且可以重复
...
//        System.out.println(name);
        ...
        /**
         * 1.针对日后处理key value 都是 String 使用 StringRedisTemplate
         * 2.针对日后处理的key value 使用 RedisTemplate
         * 3.针对于同一个key多次操作可以使用BoundxxxOps() Value List Set Zset Hash 的api 简化书写
         */
    }
...
<img src="Redis.assets/image-20200627201722700.png" alt="image-20200627201722700" style="zoom: 33%;" />
...
# 2.将redis源码包下的redis.conf拷贝到3个新目录中
...
- slave1
	port 6380
	bind 0.0.0.0
	slaveof masterip masterport   告诉从节点隶属于哪个主节点
...
- sentinel monitor 被监控数据库名字（自己起名字） ip port 1（代表整个哨兵系统哨兵的数量）
...
<img src="Redis.assets/wpsgRnQP8.jpg" alt="img" style="zoom: 25%;" />
...
- redis-cluster把所有的物理节点(master节点)映射到[0-16383]slot上,cluster 负责维护node<->slot<->value
```
...
# 2.创建集群
- ./redis-trib.rb create --replicas 1 192.168.65.129:7000 192.168.65.129:7001 192.168.65.129:7002 192.168.65.129:7003 192.168.65.129:7004 192.168.65.129:7005
```
...
- ./redis-trib.rb check 192.168.65.12:7000
...
- ./redis-trib.rb  add-node 192.168.65.129:7006  192.168.65.129:7005
- 注意:
	1.该节点必须以集群模式启动
